...
class AutoquoteAnnouncerSuite extends Properties("Autoquote announcer"):
  ...
  sealed class _PropertySpecifier() {
    def update(name: String, p: => Prop): Unit =
      val _ = property(name) = p
  }

  lazy val _property = new _PropertySpecifier()

  _property("Autoquote does not run in the morning") = forAll {
    (_year: Int, _week: Int, _day: Int, _hour: Int) =>
      ...
      val obtained = new AutoquoteAnnouncer[Id].announce()
      val expected = Nil

      obtained == expected
  }

  _property("Autoquote does not run in the evening") = forAll {
    (_year: Int, _week: Int, _day: Int, _hour: Int) =>
  ...
  _property("Autoquote does not run on the weekend") = forAll {
    (_year: Int, _week: Int, _day: Int, _hour: Int) =>
  ...
  _property("Autoquote runs during the weekday") = forAll {
    (_year: Int, _week: Int, _day: Int, _hour: Int) =>
      ...
      val obtained = new AutoquoteAnnouncer[Id].announce()
      val expected = List(Tx("irc", "#foo", None, "bar"))
