...
object Inject:
  ...
  implicit class Inject2[R0, R1, E, A](
    ...
    def inject_(r0: ZLayer[Any, E, R0]): ZIO[R1, E, A] =
  ...
  implicit class Inject5[R0, R1, R2, R3, R4, E, A](
      z: ZIO[R0 with R1 with R2 with R3 with R4, E, A]
  )(implicit t1: Tag[R1], t2: Tag[R2], t3: Tag[R3], t4: Tag[R4]):
    def inject_(
        r0: ZLayer[Any, E, R0],
        r1: ZLayer[Any, E, R1],
        r2: ZLayer[Any, E, R2],
      ...
      ZIO.environmentWithZIO { r =>
        z.provideLayer(
          r0 ++ r1 ++ r2 ++ r3 ++ ZLayer.succeedEnvironment(r)
        )
      }
