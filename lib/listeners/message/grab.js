    ...
    if (match) {
      ...
      if (sayer != from) {
        ...
        if (db.lastsaid[sayer] && db.lastsaid[sayer] !== '') {
          ...
          reply({ to: channel, message: from + ': OK - message grabbed.' });
        } else {
          reply({ to: channel, message: from + ': Sorry, ' + util.bot() + ' has not recorded anything for ' + sayer +'.'});
        }
      } else {
        reply({ to: channel, message: from + ': Sorry, you can\'t grab yourself.'});
      }
    } else {
      reply({ to: channel, message: 'Usage: @grab <username>' });
    }
  }
}
