...
function messageListener(db, from, channel, message) {
  if (/table\s*flip/i.test(message) || /flip\s*table/i.test(message)) {
  ...
  } else if (/picard/i.test(message)) {
    return [
      { to: channel, message: 'Captain Jean-Luc Picard of the USS Enterprise' },
      { to: channel, message: 'Captain Jean-Luc Picard of the USS Enterprise' },
      { to: channel, message: 'Captain Jean-Luc Picard of the USS Enterprise' },
      { to: channel, message: 'Captain Jean-Luc Picard of the USS Enterprise' },
      { to: channel, message: 'Captain Jean-Luc Picard of the USS Enterprise' },
    ...
    return [ { to: channel, message: "That's a lot of halibut." } ];
  } else if (/\$\d{3}/.test(message)) {
    return [ { to: channel, message: "That's a moderate amount of halibut." } ];
  } else if (/\$\d{2}/.test(message)) {
    return [ { to: channel, message: "That's a little bit of halibut." } ];
  } else if (/\$\d{1}/.test(message)) {
    return [ { to: channel, message: "That's hardly any halibut at all." } ];
  }
}
