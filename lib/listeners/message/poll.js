    ...
    if (poll)  {
      if (poll.isOpen) {
        if (vote === 'aye') 
          ...
          poll.nays++;
        return from + ': OK - Voted ' + vote+' on Poll ' + id +'! Current votes: ayes:' + poll.ayes + ' noes:' + poll.noes;
      } else  {
        return from + ': Sorry - Poll ' + id + ' is already closed!';
      }
    } else {
        return from + ': Sorry - Poll ' + id + ' was not found.';
    }
  } else {
    return  from + ': Usage: @vote <poll id> aye|no';
  }
  return ;
