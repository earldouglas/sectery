...
function findPoll(polls,id) {
  var poll = polls.filter(function(poll) { return (poll.id === id);});
  return poll[0] || null;
}
...
function voteListener(db, from, channel, message, messageCallback) {
  ...
  if (match) {
    ...
    msg = from + ': Usage: @vote <poll id> aye|nay';
  }
    ...
    var addPoll = function () {
      ...
      return {callback: 
        function(say) {
          ...
          channelPolls.polls.push({id: id, user: from, question: question, ayes:0, noes:0, isOpen:true,  date:util.now()});
        }
      };
    };
