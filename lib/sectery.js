...
function sectery(client) {
  ...
  setInterval(
    function() {
      for (var channel in db.quotes) {
        if (db.quotes.hasOwnProperty(channel)) {
          var nicks = [];
          for (var n in db.quotes[channel]) {
            if (db.quotes[channel].hasOwnProperty(n)) {
              nicks.push(n);
            }
          }
          var nick = nicks[Math.floor(Math.random() * (nicks.length + 1))];
          var quotes = db.quotes[channel][nick];
          var message = quotes[Math.floor(Math.random() * quotes.length)];
          if (channel && nick && message) {
