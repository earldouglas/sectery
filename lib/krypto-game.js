...
Krypto.prototype.okToGuess = function() {
  return this.guesser === null; 
};

Krypto.prototype.checkSolution = function(user, expression) {
  ...
  var regexMatch = function(regex,str) {
    var result;
    var tokens = [];
    while ((result = regex.exec(str)) !== null) {
      if (isDigit(result[0])) {
        tokens.push(parseInt(result[0]));
      } else {
  ...
  if (this.guesser != user)
    return 0;

  var tokens = regexMatch(/([\(\)\+\-\*\/]|\d+)/g,expression);
  var numbers = regexMatch(/(\d+)/g,expression);
  numbers = numbers.slice(0,5).sort();
  var cards = this.hand.slice(0,5).sort();
  ...
  correct = (correct && solution && this.hand[5][0] === solution) ;
  return correct;
};
