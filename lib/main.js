...
function loadPlugins(dir, configFile, k) {
  ...
  fs.readdirSync(__dirname + '/' + dir + '/').forEach(function(file) {
    ...
    pluginConfig.getNicknames = function(channel) {
      return Object.keys(config.nicks[channel] || {} );
    };
...
function loadListeners(client, configFile) {
  ...
  var commands = { pm: {}, message: {}, config: {}};
  ...
  client.addListener('join',function(channel, nick, message) {
    var config = fsdb.load(configFile);
    console.log(nick);
    if (!config.nicks[channel]) {
      config.nicks[channel] = {};
    } 
    config.nicks[channel][nick] = true;
    fsdb.save(configFile, config);
  });
  client.addListener('part',function(channel, nick, reason, message) {
    ...
    console.log(nick);
    if (config.nicks[channel]) {
      delete config.nicks[channel][nick];
      fsdb.save(configFile, config);
    }
  });
}
