...
function loadPlugins(dir, configFile, k) {
  ...
  fs.readdirSync(__dirname + '/' + dir + '/').forEach(function(file) {
    ...
    pluginConfig.set = function(table, key, value) {
      nconf.set('plugins:' + pluginName + ':' + table + ':' + key, value);
    };
    ...
    pluginConfig.clear = function(table, key) {
      nconf.reset('plugins:' + pluginName + ':' + table + ':' + key);
    };
...
//    var time = nconf.get('common:savetime') || 0;
//    var now = Date.now();
//    console.log('now:',now,'time',time);
//    if ( now - time > savetime) {
//      console.log('saving');
//      nconf.set('common:savetime',now);
//      nconf.save(function (err) { console.log('save done'); if (err) { return; } });
//    }
